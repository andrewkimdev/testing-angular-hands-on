...
/**
 * Fake service for the form example. This service nothing useful
...
const validateSignup = (body) => {
  ...
  const checks = {
    ...
    username: () =>
      isUsernameSyntaxValid(username) && !isUsernameTaken(username),
    email: () => isEmailSyntaxValid(email) && !isEmailTaken(email),
    password: () =>
      isPasswordSyntaxValid(password) && zxcvbn(password).score >= 3,
    address: () => !!body.address,
    name: () => isNonEmptyString(address, "name"),
    addressLine1: () =>
      plan !== "personal"
        ? isNonEmptyString(address, "addressLine1", address)
        : true,
